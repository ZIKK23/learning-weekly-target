...
-- day_of_week intentionally NOT stored: it's fully derivable from `date`
-- (TO_CHAR(date, 'FMDay')) and storing both invited drift if a bug ever
-- inserted a mismatched pair. Backend validates the incoming day name
-- against the 7 valid values before converting it to a date instead.
...
-- `status` is vestigial: the backend no longer writes or reads it. A module's
-- completion is derived at query time from submodule_progress (COUNT completed
-- submodules = total submodules) so there's a single source of truth instead of
-- two that can drift out of sync. Column kept (not dropped) since it's harmless
-- and dropping it isn't necessary for the fix.
...
-- Named submodule_time_log (not "activities") because that's what it actually is: a
-- time-log with two independent writers -- one row per submodule completion
-- (submodule_id set, from submoduleProgressController) and one row per manual
-- start/finish study session (submodule_id NULL, from activitiesController's
-- dead-from-the-UI /start+/finish endpoints).
CREATE TABLE submodule_time_log (
  id SERIAL PRIMARY KEY,
  user_id INTEGER NOT NULL,
  module_id INTEGER,
...
CREATE TRIGGER trg_activities_updated_at BEFORE UPDATE ON submodule_time_log FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
ALTER TABLE targets ADD CONSTRAINT fk_targets_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE target_days ADD CONSTRAINT fk_target_days_target FOREIGN KEY (target_id) REFERENCES targets(id) ON DELETE CASCADE;
...
ALTER TABLE submodule_time_log ADD CONSTRAINT fk_activities_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE submodule_time_log ADD CONSTRAINT fk_activities_target FOREIGN KEY (target_id) REFERENCES targets(id) ON DELETE CASCADE;
ALTER TABLE submodule_time_log ADD CONSTRAINT fk_activities_module FOREIGN KEY (module_id) REFERENCES modules(id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE submodule_time_log ADD CONSTRAINT fk_activities_submodule FOREIGN KEY (submodule_id) REFERENCES submodules(id) ON DELETE SET NULL;
ALTER TABLE daily_checkins ADD CONSTRAINT fk_daily_checkins_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE weekly_streaks ADD CONSTRAINT fk_weekly_streaks_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
...
SELECT setval(pg_get_serial_sequence('submodule_time_log','id'), COALESCE((SELECT MAX(id) FROM submodule_time_log), 1));
